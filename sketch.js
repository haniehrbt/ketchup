...
class KetchupJar {
  constructor() {
    this.x = -10; this.y = 30; this.size = 200;
    this.prevX = this.x; this.prevY = this.y; this.prevSize = this.size;
    this.amount = 0;
  }
  reset() { this.amount = 0; }
  charge() { this.amount++; }
  drawEnding() {
    background(bckColor);
    this.x += 10; this.y -= 1; this.size += 3;
    this.draw();
    this.prevX = this.x; this.prevY = this.y; this.prevSize = this.size;
  }
  shouldDrawEnding() { return this.x < W / 3; }
  draw() {
    noStroke();
    fill(bckColor);
    ...
    fill('yellow');
    rect(this.x + bottleW, this.y, this.size - 2 * bottleW, this.size * (1 - (2 / 3) * (this.amount / maxSplats)));
...
class Tomato {
  constructor() {
    this.Y0 = H + 5; this.Ax = 0; this.Ay = 0.08; this.Size = 120;
  ...
  reset() {
    ...
    this.Vx = this.Vx0; this.Vy = this.Vy0;
    this.isSplat = false; this.t = 0;
  }
  move() {
    if (!this.isSplat) {
  ...
  draw() {
    noStroke(); fill(bckColor);
    rect(this.PrevX, this.PrevY, this.Size, this.Size);
    image(this.isSplat ? splatTomatoImg : tomatoImg, this.X, this.Y, this.Size, this.Size);
  }
  isInside(x, y) {
    return x > this.X && x < this.X + this.Size && y > this.Y && y < this.Y + this.Size;
  }
  splat() {
    this.isSplat = true; jar.charge();
...
function draw() {
  if (jar.amount < maxSplats) {
    ...
    jar.draw();
    tomatoObj.draw();
    if (tomatoObj.Y < H + 10) { tomatoObj.move(); } else { tomatoObj.reset(); }
...
function mousePressed(event) {
  if (!tomatoObj.isSplat && tomatoObj.isInside(mouseX, mouseY)) {
    tomatoObj.splat();
  }
}